...
class SensorState:
    """Class to represent sensor state with proper typing"""
    def __init__(self):
        self.last_seen: Optional[datetime] = None
        self.consecutive_anomalies: int = 0
        self.recent_readings: deque = deque(maxlen=50)
...
class CableMonitor:
    ...
    def __init__(self, monitoring_interval: float = 1.0, buffer_size: int = 1000):
        ...
        self.anomaly_queue: queue.Queue = queue.Queue()
        ...
        # Per-sensor tracking with proper typing
        self.sensor_states: Dict[str, SensorState] = {}
    
    def _get_sensor_state(self, sensor_id: str) -> SensorState:
        """Get or create sensor state for given sensor ID"""
        if sensor_id not in self.sensor_states:
            self.sensor_states[sensor_id] = SensorState()
    ...
    def start_monitoring(self, network: Any, anomaly_detector: Any, background: bool = True):
    ...
    def _process_single_reading(self, reading: Dict):
        ...
        sensor_state.recent_readings.append(reading)
    ...
    def _handle_anomaly(self, reading: Dict, sensor_id: str):
        ...
        sensor_state = self._get_sensor_state(sensor_id)
        sensor_state.consecutive_anomalies += 1
    ...
    def get_sensor_summary(self) -> Dict:
        ...
        for sensor_id, state in self.sensor_states.items():
            summary[sensor_id] = {
                ...
                'recent_reading_count': len(state.recent_readings)
            }
        return summary
...
if __name__ == "__main__":
    ...
    def on_anomaly_callback(anomaly_event: Dict):
        print(f"🚨 Anomaly callback triggered: {anomaly_event['sensor_id']}")
    
    def on_alert_callback(alert: Dict):
